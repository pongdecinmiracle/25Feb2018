 ...
 <h2>Add Rule</h2> 
  ...
  <br><br>
		...
		<div>
			...
			Protocol :
			<input style="width: 20%" type=" text" id="protocol" name="protocol" placeholder="Example : TCP" >
			Signature ID :
  ...
  <input type="submit" name="submit" value="Submit" onclick="manualRule()">  
  </div>
</form>
		...
		<script type="text/javascript">
			...
			function getRule(){
				...
				$.post("http://localhost:3001/addrule", 
						{Type:action,Protocol:protocol,Sig_id:sid,Color:color}).then(function(res,status){
							console.log(res)
							if(res.success=="true"){
								alert(res.message)
							}else{
			...
			function manualRule(){
				...
				if(!manual || manual == null || manual == 0){
					...
					$.post("http://localhost:3001/addrule", 
						{action:action,Protocol:protocol,Sig_id:sig_id,Color:color}).then(function(res,status){
							console.log(res)
							if(res.success=="true"){
								alert(res.message)
							}else{
